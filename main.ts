...
class Level {
    ...
    generate() {
        ...
        function randomTile(tileProbability: Array<any>) {
            ...
            let total = 0;
            for (let j = 0; j < tileProbability.length - 1; j++) {
                let probability = tileProbability[j].probability;
                if (probability + total >= value)
                    return tileProbability[j].tile;
                total += probability;
            }
        ...
        this.itemCounts.forEach(item => {
            ...
            while (count > 0) {
                let tileLocation2 = tiles.getTileLocation(
                    randint(2, 10),
                    randint(tilemapCurrent.width, tilemapCurrent.height)
                );
                if (tiles.tileAtLocationIsWall(tileLocation2))
                    continue;
                tiles.setTileAt(tileLocation2, item.tile)
                count -= 1;
            }
        });
    }
...
class ObjSquiddy extends Obj {
    ...
    loopTiles() {
        let tileLocation3 = this.sprite.tilemapLocation();

        if (tiles.tileAtLocationEquals(tileLocation3, assets.tile`zapfish`))
            this.win();
        else if (tiles.tileAtLocationEquals(tileLocation3, assets.tile`blowfish`))
            this.getBlowfish(tileLocation3);
        else if (tiles.tileAtLocationEquals(tileLocation3, assets.tile`jellyfish`))
            this.getJellyfish(tileLocation3);
    }
...
class ObjHUD extends Obj {
    ...
    constructor(player: ObjSquiddy) {
        ...
        for (let l = 0; l < lives; l++) {
            ...
            livesSprite.x = (screen.width / 2) + (livesWidth / 2) - (l * lifeWidth) - 3;
            livesSprite.y = 116;
